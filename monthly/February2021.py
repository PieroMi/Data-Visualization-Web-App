...
def app():
    ...
    average_salesFebruary = total_sales / df['Cobrados'].sum()
    ...
    with average_column:
        ...
        st.subheader(f"US ${average_salesFebruary:,.2f}")
    ...
    if february_tab:
        ...
        february_expense_column, february_expense_pie_column = st.columns(2)
        
        with february_expense_column:
            februaryExpenses = df.groupby(by=(["Expenses"])).sum()[["Cost"]].sort_values(by="Cost")

            expenses = px.bar(
                    februaryExpenses,
                    x="Cost",
                    y=februaryExpenses.index,
                    orientation='h',
        ...
        with february_expense_pie_column:
