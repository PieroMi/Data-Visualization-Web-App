...
def app():
    ...
    average_salesJuly = total_sales / df['Cobrados'].sum()
    ...
    with average_column:
        ...
        st.subheader(f"US ${average_salesJuly:,.2f}")
    ...
    july_tab = st.sidebar.checkbox('📉Expenses')
    ...
    if july_tab:
        ...
        july_expense_column, july_expense_pie_column = st.columns(2)
        
        with july_expense_column:
            julyExpenses = df.groupby(by=(["Expenses"])).sum()[["Cost"]].sort_values(by="Cost")

            expenses = px.bar(
                    julyExpenses,
                    x="Cost",
                    y=julyExpenses.index,
                    orientation='h',
