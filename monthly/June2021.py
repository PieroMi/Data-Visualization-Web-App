...
def app():
    ...
    average_salesJune = total_sales / df['Cobrados'].sum()
    ...
    with average_column:
        ...
        st.subheader(f"US ${average_salesJune:,.2f}")
    ...
    june_tab = st.sidebar.checkbox('📉Expenses')
    ...
    if june_tab:
        ...
        june_expense_column, june_expense_pie_column = st.columns(2)
        
        with june_expense_column:
            juneExpenses = df.groupby(by=(["Expenses"])).sum()[["Cost"]].sort_values(by="Cost")

            expenses = px.bar(
                    juneExpenses,
                    x="Cost",
                    y=juneExpenses.index,
                    orientation='h',
