...
def app():
    ...
    total_cash = (report2022["Cash"].sum()) # Getting the sum for each column from the dataframe
    total_cards = (report2022["Tarjetas"].sum()) # Same logic is applied to the rest
    total_sales = (report2022["Total"].sum())
    average_sales = total_sales / report2022['Cobrado'].sum() # Total sales divided by the amount of receipts we got gives us the average sale
    ...
    with col8:
        fig0 = go.Scatter(   # Extracting data to form a Line chart demonstrating my expenses
        x = df["Fecha"],
        y = df.Expenses1,
        mode = 'markers+lines',
        name = '2021 Expenses',
