...
def app():
    ...
    average_salesAugust = total_sales / df['Cobrados'].sum()
    ...
    with average_column:
        ...
        st.subheader(f"US ${average_salesAugust:,.2f}")
    ...
    with soldPerHourColumn:
        fg_sales = df.groupby(by=["Product"]).sum()[["Hora 14" , "Hora 15" , "Hora 16" , "Hora 17" ,"Hora 18" , "Hora 19" , "Hora 20" , "Hora 21" , "Hora 22" , "Hora 23", "Hora 24"]]

        sales_hour = px.bar(
                fg_sales,
                x=fg_sales.index,
                y=["Hora 14" , "Hora 15" , "Hora 16" , "Hora 17" ,"Hora 18" , "Hora 19" , "Hora 20" , "Hora 21" , "Hora 22" , "Hora 23", "Hora 24"],
    ...
    august_tab = st.sidebar.checkbox('📉Expenses')
    ...
    if august_tab:
        ...
        august_expense_column, august_expense_pie_column = st.columns(2)
        
        with august_expense_column:
            augustExpenses = df.groupby(by=(["Expenses"])).sum()[["Cost"]].sort_values(by="Cost")

            expenses = px.bar(
                    augustExpenses,
                    x="Cost",
                    y=augustExpenses.index,
                    orientation='h',
        ...
        with august_expense_pie_column:
