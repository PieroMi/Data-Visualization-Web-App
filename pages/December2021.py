...
def app():
    ...
    st.title(":pizza: December Sales")
    st.markdown("##")

    total_cash = (df["EFECTIVO"].sum())
    total_cards = (df['Tarjetas'].sum())
    total_sales = (df['TOTAL'].sum())
    average_salesDecember = total_sales / df['Cobrados'].sum()
    ...
    with average_column:
        ...
        st.subheader(f"US ${average_salesDecember:,.2f}")
    ...
    december_tab = st.sidebar.checkbox('📉Expenses')
    ...
    if december_tab:
        ...
        december_expense_column, december_expense_pie_column = st.columns(2)
        
        with december_expense_column:
            decemberExpenses = df.groupby(by=(["Expenses"])).sum()[["Cost"]].sort_values(by="Cost")

            expenses = px.bar(
                    decemberExpenses,
                    x="Cost",
                    y=decemberExpenses.index,
                    orientation='h',
        ...
        with december_expense_pie_column:
