...
def app():
    ...
    average_salesMarch = total_sales / df['Cobrados'].sum()
    ...
    with average_column:
        ...
        st.subheader(f"US ${average_salesMarch:,.2f}")
    ...
    march_tab = st.sidebar.checkbox('📉Expenses')
    ...
    if march_tab:
        ...
        march_expense_column, march_expense_pie_column = st.columns(2)
        
        with march_expense_column:
            marchExpenses = df.groupby(by=(["Expenses"])).sum()[["Cost"]].sort_values(by="Cost")

            expenses = px.bar(
                    marchExpenses,
                    x="Cost",
                    y=marchExpenses.index,
                    orientation='h',
